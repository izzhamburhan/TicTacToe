...
class TicTacToe:
    ...
    def winner(self,square ,letter):
        ...
        # these are the only moves possibly to win a diagnonal
        if square%2 ==0:
            diagonal1 = [self.board[i] for i in [0,4,8]] #left to right diagonal
            if all([spot == letter for spot in diagonal1]) :
                 return True
            diagonal2 = [self.board[i] for i in [2,4,6]] # right to left daigonal
...
def play(game, x_player , o_player , print_game=True):
    ...
    while game.empty_squares():
        ...
        #tiny break to make things little easier
        if print_game:
            time.sleep(0,8)
